    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          >
            {hasil.warnings.map((item, i) => {
              const { title, desc } = parseWarningItem(item.pesan);
              const inputRange = [
                (i - 1) * CARD_WIDTH,
                i * CARD_WIDTH,
                (i + 1) * CARD_WIDTH,
              ...
              return (
                ...
                >
                  <Text style={styles.peringatanTitle}>{title}</Text>
                  <Text style={styles.peringatanDesc}>{desc}</Text>
                </Animated.View>
              );
            })}
          </Animated.ScrollView>
...
// ── Helper: pecah pesan "title => desc" jadi 2 bagian terpisah ──
function parseWarningItem(pesan: string): { title: string; desc: string } {
  const parts = pesan.split("=>");
  if (parts.length === 2) {
    return { title: parts[0].trim(), desc: parts[1].trim() };
  }
  return { title: pesan.trim(), desc: "" };
}
